...
fragments=response.fragments;cart_hash=response.cart_hash;$('.productItem').each(function(index,element){$(this).html('')});$('.repeaterItem .input-group').each(function(index,element){$(this).show().find('.form-control').val('')});Fancybox.show([{src:"#addtoCartPop",type:"inline"}]);$('body').trigger('added_to_cart',[fragments,cart_hash]);$(document.body).trigger('wc_fragment_refresh');$('#quickOrderFrom').unblock()});return!1});$('#stores').select2({placeholder:"Search locations",allowClear:!0});$(".single_variation_wrap").on("show_variation",function(event,variation){$('#singleProductWrap .productDescriptions .woocommerce-tabs .panel .sku_wrapper .sku').html(variation.sku);if(variation.image.url){$('#singleProductWrap .productDescriptions .woocommerce-tabs .panel .imgWrap img').remove();$('#singleProductWrap .productDescriptions .woocommerce-tabs .panel .imgWrap').append('<img src="'+variation.image.url+'" />')}})});function clearProductItems(ele,itemID){var repeaterItem=jQuery(ele).parents('.repeaterItem');repeaterItem.find('.input-group').show();repeaterItem.find('.productItem').html('');var productID=jQuery('#quickOrderFrom #bag_ids').val();var productIDArr=(productID.length>0)?productID.split(","):[];var index=productIDArr.indexOf(itemID.toString());if(index!==-1){productIDArr.splice(index,1)}
...
jQuery.ajax({type:"POST",url:CUSTOM_PARAMS.ajax_url,data:data,dataType:"JSON",beforeSend:function(response){productItem.addClass('loading');productItem.block({message:null,overlayCSS:{cursor:'none'}})},complete:function(response){productItem.removeClass('loading')},success:function(response){console.log('response',response.productQty)
if(response.html){productItem.html(response.html);thisEle.parent().parent().find('.input-group').hide().find('.form-control').val('');jQuery('.quickOrderFrom .repeater .btnsWrap .btn').removeAttr('disabled')}else{productItem.html(notFound);jQuery('.quickOrderFrom .repeater .btnsWrap .btn').attr('disabled')}
...
productItem.unblock()}})}}
function quick_order_qty_change(ele,itemID){var thisEle=jQuery(ele);var qtyVal=parseInt(thisEle.val());var productQty=jQuery('#quickOrderFrom #bag_qty').val();const productQtyArr=(productQty.length>0)?productQty.split(","):[];const productNewQtyArr=[];if(productQtyArr.length>0){productQtyArr.map(qty=>{const qtyArr=qty.split(":");if(parseInt(qtyArr[0])===parseInt(itemID)){qtyArr[1]=qtyVal}
