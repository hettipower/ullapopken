...
return;if(response.error&&response.product_url){window.location=response.product_url;return}
fragments=response.fragments;cart_hash=response.cart_hash;$('.productItem').each(function(index,element){$(this).html('')});$('.repeaterItem .input-group').each(function(index,element){$(this).show().find('.form-control').val('')});Fancybox.show([{src:"#addtoCartPop",type:"inline"}]);$('body').trigger('added_to_cart',[fragments,cart_hash]);$(document.body).trigger('wc_fragment_refresh');$('#quickOrderFrom').unblock()});return!1})});function clearProductItems(ele,itemID){var repeaterItem=jQuery(ele).parents('.repeaterItem');repeaterItem.find('.input-group').show();repeaterItem.find('.productItem').html('');var productID=jQuery('#quickOrderFrom #bag_ids').val();var productIDArr=(productID.length>0)?productID.split(","):[];var index=productIDArr.indexOf(itemID.toString());if(index!==-1){productIDArr.splice(index,1)}
