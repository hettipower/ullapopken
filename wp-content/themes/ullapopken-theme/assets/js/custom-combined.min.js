...
function scroll_anim(ele){jQuery('html, body').animate({scrollTop:jQuery(ele).offset().top},1000)}
var timeout;jQuery(function($){$('.woocommerce').on('change','input.qty',function(){if(timeout!==undefined){clearTimeout(timeout)}
timeout=setTimeout(function(){$("[name='update_cart']").trigger("click")},1000)})});(function($){"use strict";$(document.body).on("pwf_filter_js_ajax_done",function(){let pwfFilterSetting=pwffilterVariables.filter_setting;let productsContainer=pwfFilterSetting.products_container_selector;$('.wc_pro_count').html($(productsContainer+' .product').length)})})(jQuery);(function(){'use strict'
...
function remove_array_value(arr){var what,a=arguments,L=a.length,ax;while(L>1&&arr.length){what=a[--L];while((ax=arr.indexOf(what))!==-1){arr.splice(ax,1)}}
return arr}
function sku_type(ele){if(ele.value.length>8){ele.value=ele.value.substring(0,8)
ele.blur()}}