...
productItem.unblock()}})}}
function quick_order_qty_change(ele,productID,variationID){var thisEle=jQuery(ele);var qtyVal=parseInt(thisEle.val());var productQty=jQuery('#quickOrderFrom #bag_qty').val();const productQtyArr=(productQty.length>0)?productQty.split(","):[];const productNewQtyArr=[];var price=thisEle.attr('data-price');var currency=thisEle.attr('data-currency');if(productQtyArr.length>0){productQtyArr.map(qty=>{const qtyArr=qty.split(":");if(parseInt(variationID)){if(parseInt(qtyArr[0])===parseInt(productID)&&parseInt(qtyArr[2])===parseInt(variationID)){qtyArr[1]=qtyVal}}else{if(parseInt(qtyArr[0])===parseInt(productID)){qtyArr[1]=qtyVal}}
...
productNewQtyArr.push(qtyArr.join(':'));return!1})}
thisEle.attr('data-variation',response.variationID);thisEle.parent().parent().find('.color .colorSelect').attr('data-variation',response.variationID);jQuery('#quickOrderFrom #bag_qty').val(productNewQtyArr.join(','))}else{jQuery('#notifications').html('<div class="alert alert-warning d-flex align-items-center" role="alert"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/></svg><div>Product not found. Try Another.</div></div>')}
...
jQuery.ajax({type:"POST",url:CUSTOM_PARAMS.ajax_url,data:data,dataType:"JSON",beforeSend:function(response){productItem.block({message:null,overlayCSS:{cursor:'none'}})},success:function(response){if(response.variationID){if(productQtyArr.length>0){productQtyArr.map(qty=>{const qtyArr=qty.split(":");if(parseInt(variationID)){if(parseInt(qtyArr[0])===parseInt(productID)&&parseInt(qtyArr[2])===parseInt(variationID)){qtyArr[0]=response.productID;qtyArr[2]=response.variationID}}
productNewQtyArr.push(qtyArr.join(':'));return!1})}
thisEle.attr('data-variation',response.variationID);thisEle.parent().parent().find('.size .sizeSelect').attr('data-variation',response.variationID);jQuery('#quickOrderFrom #bag_qty').val(productNewQtyArr.join(','))}else{jQuery('#notifications').html('<div class="alert alert-warning d-flex align-items-center" role="alert"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/></svg><div>Product not found. Try Another.</div></div>')}
