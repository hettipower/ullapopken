...
fragments=response.fragments;cart_hash=response.cart_hash;$('.productDetails .productDetail').hide();$('.productDetails .productDetail').each(function(index,element){var variation=$(this).data('variation');if(variation==var_id){$(this).show()}});Fancybox.show([{src:"#addToCartResponseSucess",type:"inline"}]);$('body').trigger('added_to_cart',[fragments,cart_hash]);$(document.body).trigger('wc_fragment_refresh');$('form.cart').unblock()});return!1});$('.modalFooter .close').on('click',function(){Fancybox.close();return!1});Fancybox.bind('.sizeTypeClick');$('.sizeTypeClick').on('click',function(){var eleID=$(this).data('id');$('.fancybox__slide').animate({scrollTop:$(eleID).offset().top-150},2000);return!1});$('.scrollToDetails').on('click',function(){scroll_anim('.productDescriptions');return!1})});function getFormData($form){var unindexed_array=$form.serializeArray();var indexed_array={};jQuery.map(unindexed_array,function(n,i){indexed_array[n.name]=n.value});return indexed_array}
